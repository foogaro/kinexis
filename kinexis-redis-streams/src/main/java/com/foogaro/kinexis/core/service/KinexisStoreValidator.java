...
public class KinexisStoreValidator implements SmartInitializingSingleton {
    ...
    public ValidationResult validate() {
        ...
        if (properties.getProcessing().getBackpressure().getMaxInFlightPerStream() < 0) {
            errors.add("kinexis.processing.backpressure.max-in-flight-per-stream must be greater than or equal to 0");
        }
        if (properties.getProcessing().getBackpressure().getExecutorQueueSize() < 1) {
            errors.add("kinexis.processing.backpressure.executor-queue-size must be greater than or equal to 1");
        }
        if (properties.getProcessing().getBackpressure().getQueueFullBehavior() == null) {
            errors.add("kinexis.processing.backpressure.queue-full-behavior must be configured");
        }
        if (properties.getProcessing().getBackpressure().getSlowDownDelay() == null
                || properties.getProcessing().getBackpressure().getSlowDownDelay().isNegative()) {
            errors.add("kinexis.processing.backpressure.slow-down-delay must be greater than or equal to 0");
        }
        if (properties.getStream().getPartitions() < 1) {
            errors.add("kinexis.stream.partitions must be greater than or equal to 1");
        }
