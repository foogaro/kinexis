...
public class BeanFinderEntityStoreRegistry implements EntityStoreRegistry {
    ...
    public BeanFinderEntityStoreRegistry(BeanFinder beanFinder, RedisTemplate<String, String> redisTemplate) {
    ...
    public <T> Optional<CacheStore<T>> findCacheStore(Class<T> entityType) {
        ...
        return beanFinder.<T>findRepositoriesForEntity(repositoryName)
                ...
                .findFirst()
                .map(repository -> new RedisOmCacheStore<>(repositoryName, entityType, (CrudRepository<T, ?>) repository, beanFinder, java.util.Set.of(repositoryName), redisTemplate));
    }
